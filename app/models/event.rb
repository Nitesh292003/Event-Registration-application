...
class Event < ApplicationRecord
  ...
  def time_overlap?(event)
    ...
    end_datetime = DateTime.new(event_end_date.year, event_end_date.month, event_end_date.day, end_time.hour,
                                end_time.min)

    existing_event_start_datetime = DateTime.new(event.event_date.year, event.event_date.month, event.event_date.day,
                                                 event.start_time.hour, event.start_time.min)
    existing_event_end_datetime = DateTime.new(event.event_end_date.year, event.event_end_date.month,
                                               event.event_end_date.day, event.end_time.hour, event.end_time.min)
