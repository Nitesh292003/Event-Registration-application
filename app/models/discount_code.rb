...
class DiscountCode < ApplicationRecord
  ...
  validates :code,
            presence: true,
            uniqueness: { message: 'discount code already in use' },
            format: {
              with: /\A[a-zA-Z0-9]{9}\z/,
              message: 'must be exactly 9 characters using only letters and numbers'
            }
  ...
  def max_uses_must_be_positive_integer
    return unless max_uses.present? && max_uses <= 0
